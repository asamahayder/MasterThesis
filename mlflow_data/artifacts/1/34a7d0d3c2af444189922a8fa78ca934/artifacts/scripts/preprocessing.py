...
def preprocess_data(data, params):
    ...
    for pulse in tqdm(data, desc='Processing Pulses'):
        for scan in pulse['scan']:
            ...
            scan['forward_scan']['shift'] = shift_forward
            scan['backward_scan']['shift'] = shift_backward
    ...
    for i in tqdm(range(len(treated_data)), desc='Processing Treated Data'):
        ...
        all_shifts_for_treated_scans = [scan['forward_scan']['shift'] for scan in treated['scan']] + [scan['backward_scan']['shift'] for scan in treated['scan']]
        ...
        for j in range(len(all_treated_scans)):
            ...
            if i >= len(treated_data) - 4:
                ...
                reserved_data_shifts.append(all_shifts_for_treated_scans[j])
            else:
                final_data.append(all_treated_scans[j])
                labels.append(treated['samplematrix_fixed'].split()[2])
                final_data_shifts.append(all_shifts_for_treated_scans[j])

    final_data = np.asarray(final_data)
    reserved_data = np.asarray(reserved_data)
    labels = np.asarray(labels)
    reserved_labels = np.asarray(reserved_labels)
    final_data_shifts = np.asarray(final_data_shifts)
    reserved_data_shifts = np.asarray(reserved_data_shifts)
    ...
    return X, y, final_data_shifts, reserved_data, reserved_labels, reserved_data_shifts