...
def load_data(_, params):
    ...
    logger.log("Data Shape")
    logger.log("Number of samples: ", len(data))
    logger.log("Number of scans per sample: ", len(data[0]['scan']))
    ...
    logger.log("observations so far: ")
    logger.log("1) Data is randomly time shifted which shows the time instability of the topticaSmart system")
    logger.log("2) The time shift tends to group based on which day the data was sampled")
    logger.log("3) The time shift is not only across different samples, but also across multiple pulses on same samples, although to a lesser degree")
