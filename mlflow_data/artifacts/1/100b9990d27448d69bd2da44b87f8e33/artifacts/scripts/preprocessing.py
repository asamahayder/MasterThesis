...
def preprocess_data(data, params):
    ...
    def find_onset(pulse, time, threshold=0.1):
        ...
        slope = np.gradient(normalized_pulse, time)
    ...
    for pulse in tqdm(data, desc='Processing Pulses'):
        for scan in pulse['scan']:
            ...
            onset_forward = int(find_onset(signal_forward, time_forward))
            onset_backward = int(find_onset(signal_backward, time_backward))
            shift_forward = int(onset_forward - time_forward[0])
            shift_backward = int(onset_backward - time_backward[0])
