...
def preprocess_data(data, params):
    ...
    for pulse in tqdm(data, desc='Processing Pulses'):
        for scan in pulse['scan']:
            ...
            scan['forward_scan']['p2p'] = np.ptp(aligned_pulse_forward)
            scan['backward_scan']['p2p'] = np.ptp(aligned_pulse_backward)
    ...
    for i in tqdm(range(len(treated_data)), desc='Processing Treated Data'):
        ...
        all_ptp_for_treated_scans = [scan['forward_scan']['p2p'] for scan in treated['scan']] + [scan['backward_scan']['p2p'] for scan in treated['scan']]
    ...
    return X, y, final_data_ptp, reserved_data, reserved_labels, reserved_data_ptp