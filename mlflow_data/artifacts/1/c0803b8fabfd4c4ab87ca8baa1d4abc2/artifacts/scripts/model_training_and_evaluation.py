...
def train_and_evaluate_model(data, parameters):
    ...
    for train_outer_indicies, test_outer_indicies in outer_cv.split(X_train, y_train):
        ...
        results = Parallel(n_jobs=-1)(delayed(process_inner_fold)(train_inner_indicies, test_inner_indicies, X_train_outer, y_train_outer, params) for train_inner_indicies, test_inner_indicies in inner_cv.split(X_train_outer, y_train_outer))
        ...
        # Average the best hyperparameters from inner folds
        # Need to handle categorical and continuous hyperparameters separately
        best_params = {}
    ...
    # Average the final accuracies
    final_accuracy = np.mean(accuracies)
    logger.log(f"Final estimated accuracy by averaging: {final_accuracy}")

    # Filter out features based on count threshold
    ...
    # Average the final hyperparameters
    # Need to handle categorical and continuous hyperparameters separately
