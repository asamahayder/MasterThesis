...
mlflow.start_run(
    experiment_id=1, 
    run_name="Adding shift as an extra feature", 
    tags={"algorithm": "Random Forest", "domain": "Time Domain"}, 
    description="Lorenza was concerned that aligning would delete some important information. Therefore we have collected the shift and added it as an extra feature"
)
...
X, y, shifts, reserved_data, reserved_labels, reserved_shifts = run_step('preprocessing', 'preprocess_data', data, config['preprocessing']['params'])

# Step 3: Feature engineering
X_train, y_train, X_test, y_test = run_step('feature_engineering', 'feature_engineer', (X, y, shifts, reserved_data, reserved_labels, reserved_shifts), config['feature_engineering']['params'])
