...
def train_and_evaluate_model(data, parameters):
    ...
    for train_outer_indicies, test_outer_indicies in tqdm(outer_cv.split(X, y), desc='Outer CV Split', total=k_outer):
        ...
        def objective(trial):
            ...
            validation_accuracies = Parallel(n_jobs=-1)(
                delayed(inner_fold_training)(
                    train_inner_indices, test_inner_indices, X_train_outer, y_train_outer, model_for_tuning
                )
                for train_inner_indices, test_inner_indices in tqdm(inner_cv.split(X_train_outer, y_train_outer), desc='Inner CV Split', total=k_inner, leave=False)
            )
        ...
        logger.log("Outer loop true class counts:")
        ...
        logger.log("Outer loop testing predictions class counts:")
    ...
    logger.log(f"Final estimated specificity by averaging: {final_specificity}")
    logger.log(f"Final estimated sensitivity by averaging: {final_sensitivity}")
