...
def preprocess_data(data, params):
    ...
    # Function to determine the onset using the slope
    def find_onset(pulse, time, threshold=0.1):
        normalized_pulse = pulse / np.max(pulse)
        interp_func = interp1d(time, normalized_pulse, kind='cubic')
        dense_time = np.linspace(time[0], time[-1], len(time) * 10)
        dense_pulse = interp_func(dense_time)
        
        slope = np.gradient(dense_pulse, dense_time)
        onset_index = np.argmax(slope > threshold)
        
        return dense_time[onset_index]
    ...
    for pulse in tqdm(data, desc='Processing Pulses'):
        for scan in pulse['scan']:
            time_forward = scan['forward_scan']['time']
            time_backward = scan['backward_scan']['time']
            signal_forward = scan['forward_scan']['signal']
            signal_backward = scan['backward_scan']['signal']
            onset_forward = find_onset(signal_forward, time_forward)
            onset_backward = find_onset(signal_backward, time_backward)
            aligned_pulse_forward = np.roll(signal_forward, -int(onset_forward))
            aligned_pulse_backward = np.roll(signal_backward, -int(onset_backward))

            if abs(int(onset_forward)) > max_shift:
                max_shift = abs(int(onset_forward))
            if abs(int(onset_backward)) > max_shift:
                max_shift = abs(int(onset_backward))
        ...
        for ref in pulse['ref']:
            time_forward = ref['forward_scan']['time']
            time_backward = ref['backward_scan']['time']
    ...
    for pulse in tqdm(data, desc='Cutting off ends of pulses'):
        for scan in pulse['scan']:
            # cutting off each end by max_shift
