...
def train_and_evaluate_model(data, parameters):
    ...
    for train_outer_indicies, test_outer_indicies in tqdm(outer_cv.split(X_train, y_train), desc='Outer CV Split', total=k_outer):
        ...
        # Logging the count of the prediction classes
        logger.log("The following is the count of how many times each classes were predicted in the outer loop:")
        unique_labels, counts = np.unique(y_pred_outer, return_counts=True)
    ...
    # Logging the count of the prediction classes
    logger.log("The following is the count of how many times each classes were predicted:")
    unique_labels, counts = np.unique(y_pred_test, return_counts=True)
    for label, count in zip(unique_labels, counts):
        logger.log(f"{label}: {count}")
